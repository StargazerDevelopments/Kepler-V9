package org.alexdev.kepler.util.security;
...
public class RC4 {
    ...
    public RC4(int decodedKey) {
        ...
        for (int i = 0; i < 256; i++)
        {
            keySkip = ((prevKey % 19) - (i % 7));
            m *= -1;
            nKey = ARTIFICIAL_KEY[Math.abs(keyOffset + (i * m * keySkip) + keySkip) % ARTIFICIAL_KEY.length];
            prevKey = nKey;
            cKey.add(nKey);
        }
    ...
    public ByteBuf decipher(ByteBuf buffer)
    {
        ...
        for (int x = 0, z = 0; x < length; x += 2, z++)
        {
            i = (i + 1) % 256;
            j = (j + pSBox[i]) % 256;
            swapBytes();
            int d = pSBox[((pSBox[i] + pSBox[j]) % 256)];
            int t = hexToByte(data[x], data[x + 1]);
            result[z] = (byte) (t ^ d);
        }
    ...
    private static int hexToByte(byte a, byte b) {
        int value = 0;

        for (int i = 0; i < HEXALPHABET_CHARS.length; i++) {
            if (HEXALPHABET_CHARS[i] == (a & 0xff)) {
                value = (i << 4);
                break;
            }
        }
    ...
    public static int secretDecode(String secretKey) {
        String table = secretKey.substring(0, secretKey.length() / 2);
        String key = secretKey.substring(secretKey.length() / 2);
        int checkSum = 0;
